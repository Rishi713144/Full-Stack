...
const value = firstElement(["rishi","konar", 3]); //can send different types of values in inputs, without any type errors ~ this is a problem
...
const el = getFirstElement<string>(["rishikonar", "luci"]); //const el = getFirstElement(["rishikonar", "luci"]); even this works
const el2 = getFirstElement<number>([1, 2]);
const el3 = getFirstElement<string | number>(["rishikonar", "luci", 3, 4]); //const el3 = getFirstElement(["rishikonar", "luci", 3, 4]);

interface User {
  name: String
}
const el4 = getFirstElement<User>([{name:"Rishi Konar"}]);
